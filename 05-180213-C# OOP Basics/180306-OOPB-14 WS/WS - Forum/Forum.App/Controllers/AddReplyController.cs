    ...
    public class AddReplyController : IController
    {
        ...
        public MenuState ExecuteCommand(int index)
        {
            switch ((Command)index)
            {
                ...
                case Command.Submit:
                    bool validReply = PostService.TrySaveReply(this.Reply, PostView.PostId);
                    if (!validReply)
                    {
                        this.Error = true;
        ...
        public void ResetReply()
        {
            ...
            int contentLength = PostView?.Content.Count ?? 0;
            this.TextArea = new TextArea(centerLeft - 18, centerTop + contentLength - 5, TEXT_AREA_WIDTH, TEXT_AREA_HEIGHT, POST_MAX_LENGTH);
        }
